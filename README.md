...
The **Enter barcodes** list also has a narrow **On hand** column for the `MAIN` warehouse. Stock on hand is a separate stored snapshot in `data/barcode-stock-on-hand.json`: opening the tab or refreshing the product catalogue never triggers the slow stock request. It changes only when the operator chooses **Refresh stock on hand**, and the snapshot survives container restarts.

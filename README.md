...
The **Enter barcodes** list also has a narrow **Available** column for the `MAIN` warehouse. Available stock comes from the `StockAvailability` GI and is kept as a shared snapshot in `data/barcode-stock-on-hand.json`: opening the tab or refreshing the product catalogue does not trigger the stock request. The snapshot is valid for 24 hours and survives container restarts. Once it expires, stock values are hidden until an operator explicitly chooses **Refresh available stock**, refreshes it from **Manual Print**, or refreshes stock from the post-assignment label dialog. Stock refreshes run in the background and show progress against the last-known active-item count. Manual Print can use the stored `MAIN` quantity directly as the requested label quantity.
...
After a successful assignment, the operator can prepare labels for those items using the same stored `QtyAvailable` snapshot for the `MAIN` warehouse. A fresh snapshot opens the normal label review immediately without contacting MYOB. If it is missing or more than 24 hours old, the app requires the operator to use **Refresh stock first**; stock can also be refreshed on demand even while the cache is still valid.
...
LARGE_PRINTER_PRINT_SPEED=6
LARGE_PRINTER_DARKNESS=10
